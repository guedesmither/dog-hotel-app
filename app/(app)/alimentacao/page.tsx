        ...
        <div className="space-y-1">
          ...
          {openSection === 'meals' && (
            <div className="space-y-2 px-2 pb-2">
              {([
                { label: '🌅 Manhã', statusKey: 'breakfastStatus', qtyKey: 'breakfastQty' },
                { label: '☀️ Tarde', statusKey: 'lunchStatus', qtyKey: 'lunchQty' },
                { label: '🌙 Noite', statusKey: 'dinnerStatus', qtyKey: 'dinnerQty' },
                ...
                <div key={statusKey} className="flex items-center gap-3">
                      ...
                      <button key={s} onClick={() => onUpdate(statusKey, s)}
                        className={`text-xs px-2 py-1 rounded-full border font-medium transition-all ${
                          dog.report![statusKey] === s
                            ? MEAL_STATUS_COLORS[s] + ' border-transparent scale-105'
                  ...
                  <input type="text" className="w-20 shrink-0 border border-gray-200 rounded-lg px-2 py-1 text-xs focus:outline-none focus:ring-1 focus:ring-amber-400"
          ...
          {openSection === 'activities' && (
            <div className="px-2 pb-2 space-y-2">
              ...
              <div className="flex flex-wrap gap-1">
                {DEFAULT_ACTIVITIES.filter(a => !dog.report!.activities?.some(act => act.name === a)).map(a => (
                  <button key={a} onClick={() => addActivity(a)}
                    className="text-xs px-2 py-0.5 rounded-full border border-dashed border-gray-300 text-gray-500 hover:border-blue-400 hover:text-blue-600 transition-colors">
                    + {a}
                  </button>
                ))}
              </div>
              {/* Added activities */}
              {dog.report!.activities?.map(act => (
                <div key={act.id} className="flex items-center gap-2">
                  <button onClick={() => toggleActivity(act)}
                    className={`w-5 h-5 rounded border-2 shrink-0 flex items-center justify-center text-xs transition-colors ${
                      act.participated ? 'bg-green-500 border-green-500 text-white' : 'border-gray-300 hover:border-gray-400'
                    }`}>
                    {act.participated && '✓'}
                  </button>
                  <span className={`text-xs flex-1 ${act.participated ? 'text-gray-700' : 'text-gray-400 line-through'}`}>{act.name}</span>
                  <button onClick={() => deleteActivity(act.id)} className="text-gray-300 hover:text-red-400 transition-colors">
              ...
              <div className="flex items-center gap-1">
                ...
                <button onClick={() => addActivity(newActivity)} disabled={!newActivity.trim() || addingActivity}
                  className="text-xs bg-blue-500 text-white px-2 py-1 rounded-lg hover:bg-blue-600 disabled:opacity-40">
            ...
            <div className="px-2 pb-2 space-y-2">
              <div className="flex items-center gap-3">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" checked={dog.report!.hasMedication}
                    onChange={() => onUpdate('hasMedication', dog.report!.hasMedication ? '' : 'true')}
                    className="w-4 h-4 rounded border-gray-300 text-amber-500 focus:ring-amber-400" />
                  <span className="text-xs text-gray-700">Tem medicação</span>
                </label>
                {dog.report!.hasMedication && (
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" checked={dog.report!.medicationGiven === true}
                      onChange={() => onUpdate('medicationGiven', dog.report!.medicationGiven ? '' : 'true')}
                      className="w-4 h-4 rounded border-gray-300 text-green-500 focus:ring-green-400" />
                    <span className="text-xs text-gray-700">Ministrada ✓</span>
                  </label>
                )}
              </div>
              {dog.report!.hasMedication && (
                <textarea placeholder="Detalhes da medicação..."
                  ...
                  rows={2}
                  className="w-full text-xs border border-gray-200 rounded-lg px-2 py-1.5 focus:outline-none focus:ring-1 focus:ring-amber-400 resize-none" />
              )}
            </div>
          )}
          ...
          {openSection === 'notes' && (
            <div className="px-2 pb-2">
              <textarea placeholder="Como foi o dia, comportamento, observações gerais..."
